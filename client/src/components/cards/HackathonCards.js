...
function HackathonCard({ 
  ...
  return (
    ...
    >
      {/* Image - Responsive: Full width on mobile, 1/3 width on desktop */}
  ...
  <div className="absolute top-0 left-0 bg-[var(--host-primary)] text-white px-3 py-1 text-sm font-bold rounded-br-lg">
    {hackathon.difficulty}
      ...
      {/* Content - Responsive: Full width on mobile, 2/3 width on desktop */}
      <div className="w-full md:w-2/3 p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-4 mb-4">
          <div>
            <p className="text-xs md:text-sm text-gray-500">Registration Closes</p>
            <p className="font-semibold text-[var(--text-color)] text-sm md:text-base">{hackathon.lastDate}</p>
          </div>
          <div>
            <p className="text-xs md:text-sm text-gray-500">Organizer</p>
            <p className="font-semibold text-[var(--text-color)] text-sm md:text-base">{hackathon.organizer}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          <div className="flex items-center text-[var(--web-secondary)]">
            ...
            <span className="font-bold text-sm md:text-base">{hackathon.participants}</span>
          </div>
        </div>
